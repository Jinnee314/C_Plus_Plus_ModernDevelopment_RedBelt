...
void TestConstruction() {
	...
	try {
		StackVector<int, 10> k(50);
		MyUnitTest::Assert(false, "Expect invalid_argument for too large size");
	}
	catch (invalid_argument&) {
	}
	catch (...) {
		MyUnitTest::Assert(false, "Unexpected exception for too large size");
	}
}
...
void TestPushBack() {
	...
	try {
		v.PushBack(0);
		MyUnitTest::Assert(false, "Expect overflow_error for PushBack in full vector");
	}
	catch (overflow_error&) {
	}
	catch (...) {
		MyUnitTest::Assert(false, "Unexpected exception for PushBack in full vector");
	}
}
...
void TestPopBack() {
	...
	for (int i = static_cast<int>(v.Size()); i > 0; --i) {
		ASSERT_EQUAL(v.PopBack(), i);
	}

	try {
		v.PopBack();
		MyUnitTest::Assert(false, "Expect underflow_error for PopBack from empty vector");
	}
	catch (underflow_error&) {
	}
	catch (...) {
		MyUnitTest::Assert(false, "Unexpected exception for PopBack from empty vector");
	}
}
...
int main() {
	...
	std::cerr << "Running benchmark..." << endl;
